...
O artigo apresenta ainda, uma comparação entre tempo de processamento, consumo energético e custo (tempo x recursos) de cada estratégia de virtualização. O DoCB foi avaliado com a extensão de um modelo chamado AutoElastic \citetexto{7090978} para suportar contêineres, o qual foi utilizado para fazer o gerenciamento das execuções com contêineres e máquinas virtuais na Nuvem. Os dados gerados pelo AutoElastic são utilizados para compor e comparar os resultados de ambas tecnologias. 
...
\label{comparacao}
A Tabela \ref{tab:table2} apresenta um resumo dos trabalhos detalhados na subseção \ref{state}, elencando as principais características dos trabalhos conduzidos. Podemos notar que, apesar do trabalho \citetexto{7090978} apresentar um modelo elástico para aplicações PAD, não existem outros estudos que mostrem o comportamento de contêineres sob estas condições. Os demais trabalhos fazem uma análise do desempenho de contêineres, porém sem instanciação dinâmica de contêineres pelo gerenciador, o que daria o aspecto de um sistema elástico.
