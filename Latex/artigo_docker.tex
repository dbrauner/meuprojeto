...
As técnicas de análise de \textit{Speedup} Elástico e Eficiência Elástica serão utilizadas para medir o desempenho de elasticidade de de nuvem em aplicações HPC \cite{Facco2016}. O \textit{Speedup} Elástico será utilizado para identificar a elasticidade do modelo em duas situações, uma com modelo configurado para um processo único executando a aplicação, e no outro será analisado o desempenho da execução em paralelo. A técnica de análise de Eficiência Elástica será utilizada, pois é pertinente identificar o qual eficiente é o modelo em questão de utilização dos recursos disponíveis, considerando sempre a elasticidade horizontal, ou seja, adicionando ou removendo recursos através de provisionamento de novas instâncias. 

A partir da utilização de tais técnicas de avaliação, pode-se considerar vários cenários de implementação da nuvem, ou seja, as combinações serão exploradas independente de quantos núcleos de processamento estão disponível para cada nó computacional, isto com o objetivo de identificar as situações em que os modelos terão comportamentos discrepantes. Dessa forma, será feita também uma análise comparando o tempo de provisionamento dos recursos, levando em consideração todos os momentos envolvidos, que são: a busca da imagem no nó principal, o tempo de transferência desta imagem para o nó de execução, o tempo de criação da instância a partir da imagem e o tempo de inicialização da instância para rodar a aplicação. Estes passos são considerados tanto para máquinas virtuais quanto containers. Serão utilizadas, também, aplicações de \textit{benchmark}, propostas pela NASA \citetexto{NASA2016}, para avaliar o desempenho de aplicações paralelas. 
...
A partir da revisão bibliográfica que foi apresentada no capítulo \ref{fundamentacao}, foi possível estabelecer as bases necessárias para que pudesse ser visto as oportunidades de melhoria de tecnologias de virtualização ambientes com elasticidade. A partir disto, no capítulo \ref{related}, estudamos os trabalhos relacionados que abordam, tanto os conceitos de virtualização baseada em containers em comparação à virtualização baseada em \textit{hypervisor}, quanto os modelos de elasticidade para aplicações HPC. Através disto, traçamos um paralelo entre estas pesquisas e identificamos as lacunas existentes nestes campos de pesquisa. A análise dos trabalhos permitiu identificar uma melhoria no modelo de elasticidade atual baseado em virtualização por \textit{hypervisor}, além de despertar questões a repeito de que em quais momentos uma tecnologia pode ser mais indicada que a outra. Assim, poderemos utilizar das métricas de avaliação propostas na seção \ref{avaliacao} para identificar os \textit{overheads} de cada tecnologia. Neste capítulo final, pretendemos mostras as contribuições esperadas deste trabalho (sessão \ref{contribuicoes}), além de descorrer sobre as oportunidades de trabalhos futuros, na seção \ref{futuros}. Finalmente, na seção \ref{cronograma}, mostraremos o planejamento para os próximos passos de implementação do projeto proposto, através de um cronograma.
...
As próximas etapas do trabalho irão compor a implementação do modelo proposto e das análises comparativas, a partir da utilização de técnicas de \textit{benchmark}. Pretende-se, então estudar o desempenho de aplicações HPC, sob diferentes configurações, no modelo de elasticidade com virtualização baseada em containers e em \textit{hypervisor}, identificando sob quais circunstâncias uma tecnologia pode ser mais interessante do que outra, dependendo da finalidade da implementação.

Na sessão \ref{cronograma}, podemos verificar quais serão as etapas de continuação deste trabalho para conseguir alcançar os objetivos propostos. Como trabalhos futuros, também pretende-se identificar os overheads de desempenho de memória, CPU, operações de I/O e rede de cada tecnologia.
...
Estudo e e escolha de testes de benchmark e profiling            		& X &   &   &   &   & 			\\ \hline
Modelagem da solução			 										& X & X &   &   &   &			\\ \hline
Definir estratégia para comparação de resultados						&   & X &   &   &   & 			\\ \hline
Desenvolvimento da extensão do modelo aplicando containers   			& 	& X &	&   &   &			\\ \hline
Testes e avaliação do modelo sob diferentes cenários					& 	& X & 	& 	& 	&  		   \\ \hline
